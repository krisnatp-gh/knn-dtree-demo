        ...
        with st.expander("📊 Odds Ratio Analysis"):
            ...
            if model_type == 'unregularized':
                beta_table = pd.DataFrame({
                    'Coefficient': ['β<sub>0</sub>', 'β<sub>Age</sub>', 'β<sub>Annual Income</sub>'],
                    'Value': [format_number(cached_model.params['const']),
                             format_number(cached_model.params['Age']),
                             format_number(cached_model.params['Annual_Income_IDR'])]
                })
            else:  # regularized
                beta_table = pd.DataFrame({
                    'Coefficient': ['β<sub>0</sub>', 'β<sub>Age</sub>', 'β<sub>Annual Income</sub>'],
                    'Value': [format_number(cached_model.intercept_[0]),
                             format_number(cached_model.coef_[0][0]),
                             format_number(cached_model.coef_[0][1])]
                })
